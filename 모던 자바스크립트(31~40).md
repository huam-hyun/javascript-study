...
exec 메서드는 모든 패턴을 검색하는 플래그를 지정해도 첫 번째 매칭 결과만 반환한다.
...
만약 자바스크립트 코드에 DOM이나 CSSOM을 변경하는 DOM API가 사용된 경우 DOM이나 CSSOM이 변경된다. 이때 변경된 DOM과 CSSOM은 다시 렌더 트리로 결합되고 이를 기반으로 브라우저 환경을 다시 렌더링한다. 이를 리플로우, 리페인트라 한다.
...
DOM API인 document.getElementById('apple')은 DOM에서 id가 'apple'인 HTML 요소를 취득하는데, 실행 시점에 아직 요소가 파싱되지 않았기 때문에 위 예제는 정상적으로 작동하지 않는다.
...
노드 객체는 총 12개의 종류가 있다. 이 중에서 중요한 노드 타입은 다음 4가지 이다.



##### 문서 노드(document node)

DOM트리의 최상위에 존재하는 루트 노드로서 document 객체를 가리킨다. document 객체는 브라우저가 렌더링한 HTML 문서 전체를 가리키는 개체로서 전역 객체 window의 document 프로퍼티에 바인딩되어 있다. 따라서 window.document 또는 document로 참조할 수 있다.



##### 요소 노드(element node)

HTML 요소를 가리키는 객체다. 요소 노드는 중첩에 의해 부자 관계를 가지며, 이 관계를 통해 정보를 구조화한다. 문서의 구조를 표현한다고 할 수 있다.



##### 어트리뷰트 노드(attribute node)

HTML 요소의 어트리뷰트를 가리키는 객체다. 어트리뷰트가 지정된 요소 노드와 연결되어 있다. 단, 요소 노드는 부모 노드와 연결되어 있지만 어트리뷰트 노드는 요소 노드에만 연결되어 있다. 즉, 요소 노드와 어트리뷰트 노드는 형제 노드가 아니다.



##### 텍스트 노드(text node)

HTML 요소의 텍스트를 가리키는 객체다. 요소 노드가 문서의 구조를 표현한다면 텍스트 노드는 문서의 정보를 표현한다고 할 수 있다. 텍스트 노드는 요소 노드의 자식 노드이며, 리프 노드다.



위 4가지 노드 이외에도 주석을 위한 Comment 노드, DOCTYPE을 위한 Document Type 노드, 복수의 노드를 생성하여 추가할 때 사용하는 DocumentFragment 노드 등 총 12개의 노드 타입이 있다.
...
노드 객체에는 노트 타입에 상관없이 모든 노드 객체가 공통으로 갖는 기능도 있고, 타입에 따라 고유한 기능도 있다.
...
id 값은 HTML 문서 내의 유일한 값이지만 여러 개가 존재할 수 있다. 이때 getElementById 메서드는 맨 처음으로 찾은 요소만 반환한다. 만약 id값을 갖는 HTML 요소가 없을 경우 null을 반환한다.
...
```js
const $all = cocument.getELementsByTagName('*');
```

getElementsByTagName 메서드는 Document.prototype에 정의된 메서드와 Element.prototype에 정의된 메서드가 있는데 Document에 정의된 메서드는 DOM 전체에서 검색하고 Element에 정의된 메서드는 요소 노드에서 호출하여, 자손 노드 중에서 탐색한다.
...
Document.prototype/Element.prototype.getElementsByClassName 메서드는 인수로 전달한 class 어트리뷰트 값을 갖는 모든 요소 노드들을 탐색하여 반환한다. 결과로 HTMLCollection 객체를 반환한다.
...
태그 이름을 이용한 요소 노드 취득과 마찬가지로 Document.prototype의 getElementsByClassName 메서드는 DOM 전체에서 탐색하고 Element.prototype의 메서드는 자손 노드 중에서 요소 노드를 탐색하여 반환한다.

인수로 전달된 class 값을 갖는 요소가 존재하지 않는 경우 메서드는 빈 HTMLCollection 객체를 반환한다.
...
/* 가상 요소 선택자: p 요소의 콘텐츠의 앞에 위치하는 공간을 선택. 일반적으로 content 프로퍼티와 함꼐 사용된다 */
p::before {...}
```
...
querySelectorAll 메서드는 인수로 전달한 CSS 선택자를 만족시키는 모든 요소 노드를 탐색하여 반환한다. querySelectorAll 메서드는 여러 개의 요소 노드 객체를 갖는 DOM 컬렉션 객체인 NodeList 객체를 반환한다.
...
<html>
    ...
    <body>
        ...
        <script>
            // ul 요소의 자식 요소인 li 요소들을 탐색하여 반환한다.
...
CSS 선택자로 요소 노드를 취득하는 것은 getElementById, getElementBy*** 메서드보다 느리다고 알려져 있지만, 구체적이고 일관된 방식으로 요소 노드를 취득할 수 있다는 장점이 있다.
...
HTMLCollection과 NodeList의 중요한 특징은 노드 객체의 상태 변화를 실시간으로 반영하는 Live 객체라는 것이다. HTMLCollection 의 경우 항상 Live 객체지만 NodeList는 대부분의 경우 non-live 객체지만 때때로 live 객체로 동작한다.
...
위 예제를 확인해보면 두 번째 요소가 바뀌지 않은 것을 볼 수 있다.

![image-20220321174706155](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220321174706155.png)

이유는 HTMLCollection이 live객체이기 때문이다. for문의 코드 블록을 분석해보자.

1. 첫 번째 반복 (i === 0)

   $elem[0]은 첫 번째 li 요소다. 이 요소의 class 값이 'red'에서 'blue'로 변경이되면 실시간으로 $elems에서 제거된다.

2. 두 번째 반복 (i === 1)

   첫 번째 반복에서 첫 번째 li 요소가 제거되었으니 $elems[1]의 요소는 세 번째 li이다. 따라서 세 번째 li 요소의 class가 blue로 바뀌고 $elems에서 제거된다.

3. 세 번째 반복 (i === 2)

   $elems에는 두 번째 li요소만 남아있는 상태이다. 따라서 $elems.length는 1이므로 for문의 조건에 부합하지 않으므로 그대로 반복문을 빠져나오게 된다.



이처럼 HTMLCollecton 객체는 실시간으로 노드 객체의 상태 변경을 반영하여 요소를 제거할 수 있기 때문에 HTMLCollection 객체를 for 문으로 순회할 때 조심해야 한다. 이 문제는 for문을 역방향으로 순회하는 방법으로 피할 수 있다.
...
더 간단한 해결책은 HTMLCollection 객체를 사용하지 않는것이다.
...
querySelectorAll 메서드는 NodeList 객체를 반환한다. 이때 NodeList객체는 non-live 객체이다.
...
NodeList 객체는 대부분의 경우 non-live 객체로 동작하지만, childNodes 프로퍼티가 반환하는 NodeList 객체는 live 객체로 동작하므로 주의가 필요하다.
...
<html>
    ...
    <script>
        ...
        for(let i = 0; i < childNodes.length; i++){
            // removeChild 메서드가 호출될 때마다 NodeList 객체가 실시간으로 변경된다.
            // 따라서 1, 3, 5번째 요소만 삭제된다.
            $fruits.removeChild(childNodes[i]);
        }
        
        // 예상과는 다르게 1, 3, 5번째 노드만 삭제되었다.
...
요소 노드를 취득한 다음, 취득한 요소 노드를 기점으로 DOM 트리의 노드를 옮겨 다니며 탐색해야 하는 경우가 있다.
...
ul#fruits 요소는 3개의 자식 요소를 가진다. 이때 ul#fruits의 요소를 취득한 다음, 자식 노드를 모두 탐색하거나 자식 노드 중 하나만 탐색할 수 있다.

li.banana 요소는 2개의 형제 요소와 부모 요소를 갖는다. 이때 li.banana의 요소를 취득한 다음, 형제노드나 부모 노드를 탐색할 수 있다.

이처럼 DOM 트리 상의 노드를 탐색할 수 있도록 Node, Element 인터페이스는 트리 탐색 프로퍼티를 제공한다.

parentNode, previousSibling, firstChild, childNodes 프로퍼티는 Node.prototype이 제공하고, Element가 포함된 previousElementSibling, nextElementSibling, children 프로퍼티는 Element.prototype이 제공한다.

노드 탐색 프로퍼티는 모두 접근자 프로퍼티다. 단, getter만 존재하기 때문에 값을 할당하면 아무런 에러 없이 무시된다.



#### 39.3.1 공백 텍스트 노드

HTML 요소 사이에 공백 문자가 생성하는 노드를 공백 텍스트 노드라 한다.

HTML 문서에 스페이스, 탭, 엔터 등을 입력하면 공백 문자가 추가된다. 그러면 DOM에도 공백 텍스트 노드가 추가된다. 인위적으로 HTML 문서의 공백 문자를 제거하면 공백 텍스트 노드를 생성하지 않지만, 가독성이 안좋아지니 하지말자.
...
| 프로퍼티                            | 설명                                                         |
| ----------------------------------- | ------------------------------------------------------------ |
| Node.prototype.childNodes           | 자식 노드를 모두 탐색하여 DOM 컬렉션 객체인 NodeList에 담아 반환한다. 여기에는 요소 노드 뿐만 아니라 텍스트 노드도 포함되어 있을 수 있다. |
| Element.prototype.children          | 자식 노드 중에서 요소 노드만 모두 탐색하여 HTMLCollection에 담아 반환한다. 텍스트 노드는 포함되지 않는다. |
| Node.prototype.firstChild           | 첫 번째 자식 노드를 반환한다. 반환한 노드는 텍스트 노드이거나 요소 노드다. |
| Node.prototype.lastChild            | 마지막 자식 노드를 반환한다. 반환한 노드는 텍스트 노드이거나 요소 노드다. |
| Element.prototype.firstElementChild | 첫 번째 자식 노드를 반환한다. 반환한 노드는 요소 노드만 반환한다. |
| Element.prototype.lastElementChild  | 마지막 자식 요소 노드를 반환한다. 반환한 노드는 요소 노드만 반환한다. |
...
<html>
    ...
    <script>
        ...
        // 모든 자식 노드를 탐색한다(NodeList)
        console.log($fruits.childNodes);
        // NodeList(7) [text, li.apple, text, li.banana, text, li.orange, text]
        
        //모든 자식 노드를 탐색한다(HTMLCollection)
        console.log($fruits.children);
        // HTMLCollection(3) [li.apple, li.banana, li.orange]
        
        // 첫 번째 자식 노드를 탐색한다. 텍스트 노드를 반환할 수 있다.
        console.log($fruits.firstChild);
        // #text
        
        // 마지막 자식 노드를 탐색한다. 텍스트 노드를 반환할 수 있다.
        console.log($fruits.lastChild);
        // #text
        
        // 첫 번째 자식 노드를 탐색한다. 요소 노드만 반환할 수 있다.
        console.log($fruits.firstElementChild);
        // li.apple
        
        // 마지막 자식 노드를 탐색한다. 요소 노드만 반환할 수 있다.
...
![image-20220322165027590](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322165027590.png)



#### 39.3.3 자식 노드 존재 확인

자식 노드가 존재하는지 확인하려면 Node.prototype.hasChildNodes 메서드를 사용한다. 자식 노드가 존재하면 true, 존재하지 않으면 false를 반환한다. 단, 텍스트 노드를 포함하여 자식 노드의 존재를 확인한다.
...
요소 노드의 텍스트 노드는 자식 노드이다. 따라서 firstChild  프로퍼티로 접근할 수 있다. firstChild 프로퍼티는 첫 번째 자식 노드를 반환한다.
...
형제 노드를 탐색하려면 다음과 같은 노드 탐색 프로퍼티를 사용한다. 단, 어트리뷰트 노드는 부모 노드가 같은 형제 노드가 아니기 때문에 반환되지 않는다.

| 프로퍼티                                 | 설명                                                         |
| ---------------------------------------- | ------------------------------------------------------------ |
| Node.prototype.previousSibling           | 부모 노드가 같은 형제 노드 중에서 자신의 이전 형제 노드를 탐색하여 반환한다. 반환된 노드는 요소 또는 텍스트 노드이다. |
| Node.prototype.nextSibling               | 부모 노드가 같은 형제 노드 중에서 자신의 다음 형제 노드를 탐색하여 반환환다. 반환된 노드는 요소 또는 텍스트 노드이다. |
| Element.prototype.previousElementSibling | 부모 노드가 같은 형제 노드 중에서 자신의 이전 형제 노드를 탐색하여 반환한다. 요소 노드만 반환된다. |
| Element.prototype.nextElementSibling     | 부모 노드가 같은 형제 노드 중에서 자신의 다음 형제 노드를 탐색하여 반환한다. 요소 노드만 반환된다. |
...
<html>
    ...
    <script>
        ...
        // #fruits의 첫 번째 자식 노드를 탐색한다. 텍스트 노드도 가능하다.
        const {firstChild} = $fruits;
        console.log(firstChild);	// #text
		
        // $fruits의 첫 번째 자식 노드의 다음 형제 노드를 검색한다.
        const {nextSibling} = firstChild;
        console.log(nextSibling);	// li.apple

        // li.apple 요소의 이전 형제 노드를 탐색한다. 텍스트 노드도 가능하다.
        const {previousSibling} = nextSibling;
        console.log(previousSibling);	// #text

        // #fruits의 첫 번째 자식 노드를 탐색한다. 요소 노드만 탐색한다
        const {firstElementChild} = $fruits;
        console.log(firstElementChild);	// li.apple

        // #fruits 요소의 첫 번째 자식 요소 노드의 다음 형제 노드를 탐색한다. 요소 노드만 반환한다.
        const {nextElementSibling} = firstElementChild;
        console.log(nextElementSibling);	// li.banana

        // li.banana 요소의 이전 형제 노드를 탐색한다. 요소 노드만 반환한다.
...
![image-20220322172117317](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322172117317.png)



### 39.4 노드 정보 취득

노드 객체에 대한 정보를 취득하려면 다음과 같은 프로퍼티를 사용한다.

| 프로퍼티                | 설명                                                         |
| ----------------------- | ------------------------------------------------------------ |
| Node.prototype.nodeType | 노드 객체의 종류(타입)를 나타내는 상수를 반환한다.<br />• Node.ELEMENT_NODE: 요소 노드 타입을 나타내는 상수 1을 반환<br />• Node.TEXT_NODE: 텍스트 노드 타입을 나타내는 상수 3을 반환<br />• Node.DOCUMENT_NODE: 문서 노드 타입을 나타내는 상수 9를 반환 |
...
![image-20220322172914587](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322172914587.png)
...
노드 객체의 nodeValue 프로퍼티를 참조하면 노드 객체의 값을 반환한다. 이 값이란 텍스트 노드의 텍스트다. 따라서 텍스트 노드가 아닌 경우 null을 반환한다.
...
<html>
    ...
    <script>
    	console.log(document.nodeValue);	// 문서 노드이기 때문에 null을 반환한다.
...
Node.prototype.textContent 프로퍼티는 setter와 getter 모두 존재하는 접근자 프로퍼티다. 요소 노드의 텍스트와 모든 자손 노드의 텍스트를 모두 취득하거나 변경한다.

textContent 프로퍼티를 참조하면 요소 노드의 콘텐츠 영역 내의 텍스트를 모두 반환한다. 이때 HTML 마크업은 무시된다.
...
앞서 살펴본 nodeValue 프로퍼티를 참조하여도 텍스트를 얻을 수 있지만, 텍스트 노드가 아닌 노드의 nodeValue 프로퍼티는 null을 반환하므로 의미가 없었다. nodeValue 프로퍼티를 사용하면 textContent 프로퍼티를 사용할 때보다 코드가 더 복잡해진다.
...
![image-20220322175538534](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322175538534.png)



참고로 textContent 프로퍼티와 유사한 동작을 하는 innerText 프로퍼티가 있다. innerText는 다음과 같은 이유로 사용하지 않는 것이 좋다.

- CSS에 순종적이다. 예를 들ㅇ, CSS에 의해 비표시로 지정된 요소 노드의  텍스트를 반환하지 않는다.
...
DOM 조작은 새로운 노드를 생성하여 DOM에 추가하거나 기존 노드를 삭제 또는 교체하는 것을 말한다. DOM 조작에 의해 리플로우와 리페인트가 발생하는 원인이 되므로 성능에 영향을 준다. 따라서 DOM 조작은 주의해서 다루어야된다.



#### 39.6.1 innerHTML

Element.prototype.innerHTML 프로퍼티는 setter와 getter 모두 존재하는 접근자 프로퍼티로 요소 노드의 HTML 마크업을 취득하거나 변경한다. 요소 노드의 innerHTML 프로퍼티를 참조하면 콘텐츠 영역 내에 포함된 모든 HTML 마크업을 문자열로 반환한다.
...
앞서 살펴본 textContent는 HTML 마크업을 무시하고 텍스트만 반환했지만 innerHTML은 마크업이 포함된 문자열을 그대로 반환한다.



innerHTML에 문자열을 할당하면 HTML 마크업이 파싱되어 요소 노드의 자식 노드로 DOM에 반영된다.
...
![image-20220322180153900](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322180153900.png)

이처럼 innerHTML 프로퍼티를 사용하면 HTML 마크업 문자열로 간단히 DOM 조작이 가능하다.

하지만 사용자에게 입력받은 데이터를 그대로 innerHTML 프로퍼티에 할당하는 것은 크로스 사이트 스크립팅 공격에 취약하다. 자바스크립트 악성 코드가 포합되어 있다면 파싱 과정에서 그대로 실행될 가능성이 있기 때문이다.
...
HTML5 에서는 innerHTML 프로퍼티로 삽입된 script 요소 내의 자바스크립트 코드를 실행하지 않기 때문에 HTML5를 지원하는 브라우저에서는 통하지 않는다. 하지만 script 요소 없이도 크로스 사이트 스크립팅 공격은 가능하다.
...
innerHTML의 또 다른 단점은 모든 자식 노드를 제거한 다음 DOM을 변경한다는 것이다. `<li class="apple">Apple</li>`를 유지하고 `<li class="banana">Banana</li>`만 추가한다고 했을 때 innerHTML을 사용하면 apple을 제거하고 다시 apple과 banana를 만든다.

또 innerHTML 프로퍼티는 새로운 요소를 삽일할 때 삽입될 위치를 지정할 수 없다는 단점도 있다.
...
DOMString은 HTML 마크업을 파싱하고 그 결과로 생성된 노드를 position 위치에 삽입하여 DOM에 반영한다. 첫 번째 인수로 전달할 수 있는 문자열은 'beforebegin', 'afterbegin', 'beforeend', 'afterend' 4가지다.
...
<html>
    ...
    <script>
        ...
        // 2. 텍스트 노드 생성
        const textNode = documentcreateTextNode('Banana');
...
Document.prototype.createElement(tagName) 메서드는 새로운 요소 노드를 반환한다. tagName에는 태그 이름을 나타내는 문자열을 인수로 전달한다.

```js
const $li = document.createElement('li');
```

생성된 요소 노드는 DOM에 추가되지 않고 홀로 존재하는 상태다. 따라서 이후 노드를 DOM에 추가하는 별도 처리가 필요하다. 또한 아무런 자식 노드를 갖지 않기 때문에 텍스트 노드도 존재하지 않는다.
...
createTextNode 메서드로 생성한 텍스트 노드도 홀로 존재하는 상태기 때문에 요소 노드에 추가하는 별도의 처리가 필요하다.



##### 텍스트 노드를 요소 노드의 자식 노드로 추가

Node.prototype.appendChild(childNode) 메서드는 매개 변수 childNode에게 인수로 전달한 노드를 appendChild 메서드를 호출한 노드의 마지막 자식 노드로  추가한다.(childNode를 Node 마지막에 추가한다)

```js
// textNode를 $li 마지막 자식 노드로 추가
$li.appendChild(textNode);
```
...
Node.prototype.appendChild 메서드를 사용하여 생성한 요소 노드를 #fruits 요소 노드의 마지막 자식 요소로 추가한다.
...
이 예제는 3번을 반복하기 때문에 DOM이 3번 변경된다. 리플로우와 리페인트가 3번 실행되기 때문에 성능에 좋지 않다.



이런 경우 컨테이너 요소를 사용하면 좋다. 추가할 3개의 요소를 컨테이너에 추가하고 컨테이너를 최종적으로 DOM에 추가하는 것이다.
...
하지만 이 예제도 불필요한 div 요소가 DOM에 추가되니 바람직하지 않다.



이러한 문제는 DocumentFragment 노드를 통해 해결할 수 있다. 이 노드는 노드 객체의 일종으로 ,부모 노드가 없어서 기존 DOM과는 별도로 존재한다는 특징이 있다.

DocumentFragment노드에 자식 노드를 추가하여도 기존 DOM에는 변경이 발생하지 않는다. 또한 DocumentFragment노드를 DOM에 추가하면 DocumentFragment노드는 없어지고 자식 노드들만 DOM에 추가된다.
...
Node.prototype.appendCHild 메서드는 인수로 전달받은 노드를 호출한 노드 맨 마지막 자식 노드로 추가한다.



##### 지정한 위치에 노드 삽입

Node.prototype.insertBefore(newNode, childNode) 메서드는 newNode를 childNode 앞에 삽입한다.
...
두 번째 인수로 전달받은 노드는 반드시 insertBefore 메서드를 호출한 노드의 자식 노드여야 한다. 그렇지 않으면 DOMException 에러가 발생한다. 또한 두 번째 인수가 null이면 appendChild와 동일하게 작동한다.



#### 39.6.6 노드 이동

DOM에 이미 존재하는 노드를 appendChild 또는 insertBefore 메서드를 사용하여 DOM에 다시 추가하면 현재 위치에서 제거하고 새로운 위치에 노드를 추가한다.
...
Node.prototype.cloneNode([deep: true | false])메서드는 노드의 사본을 생성하여 반환한다. 매개변수에 true를 전달하면 깊은 복사를 하여 자손 노드가 포함된 사본을 생성하고, false를 전달하거나 생략하면 얕은 복사로 노드 자신만의 사본을 생성한다. 얕은 복사의 경우 텍스트 노드도 없다.
...
![image-20220322184344224](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322184344224.png)



#### 39.6.8 노드 교체

Node.prototype.replaceChild(newChild, oldChild) 메서드는 자신이 호출한 노드의 자식 노드를 다른 노드로 교체한다. 이때 oldChild 노드는 DOM에서 제거된다.
...
![image-20220322184608517](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322184608517.png)



#### 39.6.9 노드 삭제

Node.prototype.removeChild(child) 메서드는 child를 DOM에서 삭제한다.
...
![image-20220322184715807](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322184715807.png)
...
HTML 요소는 여러 개의 어트리뷰트(속성)를 가질 수 있다. 어트리뷰트는 태그에 `어트리뷰트 이름="어트리뷰트 값"`형식으로 정의한다.

글로벌 어트리뷰트와 이벤트 핸들러 어트리뷰트는 모든 HTML 요소에서 공통적으로 사용 가능하지만 특정 요소에만 사용가능한 어트리뷰트도 있다.

HTML 문서가 파싱될 때 요소의 어트리뷰트는 어트리뷰트 노드로 변환(어트리뷰트 당 1개)되어 요소 노드와 연결된다. 어트리뷰트 노드의 참조는 유사 배열 객체이자 이터러블인 NamedNodeMap 객체에 담겨서 요소 노드의 attributes 프로퍼티에 저장된다.

Element.prototype.attributes 프로퍼티에 접근하면 어트리뷰트를 취득할 수 있고, getter만 존재하는 접근자 프로퍼티다. 요소의 모든 어트리뷰트 노드의 참조가 담긴 NamedNodeMap 객체를 반환한다.
...
![image-20220322185410619](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322185410619.png)
...
참조는 Element.prototype.getAttribute(attributeName) 메서드, 변경은 Element.prototype.setAttribute(attributename, attributeValue) 메서드를 사용한다.
...
요소 노드 객체에는 어트리뷰트에 대응하는 프로퍼티가 존재한다. 이 DOM 프로퍼티들은 어트리뷰트 값을 초기값으로 가지고 있다.

DOM 프로퍼티는 setter와 getter모두 가지고 있기 때문에 참조와 변경이 가능하다. 그렇다면 HTML 어트리뷰트는 DOM에서도 변경 가능하니 중복 관리되는 것처럼 보인다.
...
**HTML 어트리뷰트의 역할은 요소의 초기 상태를 지정하는 것이다. 즉, HTML 어트리뷰트 값은 요소의 초기 상태를 의미하고 이는 바뀌지 않는다.**



만약 input 태그가 있고 첫 렌더링 이후 사용자가 input의 내용을 바꾸면 초기 상태와 바뀌게 된다. 그럼 HTML 어트리뷰트의 값이 소용 없는건가 할 수 있지만, 초기 상태를 관리하지 않으면 웹페이지를 처음 표시하거나 새로고침할 때 초기 상태를 표시할 수 없다.

따라서 요소 노드는 초기 상태와 최신 상태를 관리해야 한다. 초기 상태는 어트리뷰트 노드가 관리하며, 요소 노드의 최신 상태는 DOM 프로퍼티가 관리한다.



##### 어트리뷰트 노드

HTML 어트리뷰트로 지정한 HTML 요소의 초기 상태는 어트리뷰트 노드에서 관리한다. 어트리뷰트 값은 사용자의 입력에 의해 상태가 변경되어도 변하지 않고 지정한 HTML 요소의 초기 상태를 그대로 유지한다.

어트리뷰트 노드가 관리하는 값을 취득하거나 변경하려면 getAttribute/setAttribute 메서드를 사용한다.
...
DOM 프로퍼티에 값을 할당하는 것은 사용자가 상태를 변경하는 행위와 같다. 이때 HTML 요소에 지정한 어트리뷰트 값에는 어떠한 영향도 주지 않는다. 단, 모든 DOM 프로퍼티가 최신 상태를 관리하는 것은 아니다. 예를 들어, id 어트리뷰트의 경우 사용자의 입력에 상관없이 항상 일정하게 유지된다. 따라서 이러한 프로퍼티들은 어트리뷰트 값이 변하면 프로퍼티 값이 변하고 반대도 마찬가지이다.



##### HTML 어트리뷰트와 DOM 프로퍼티의 대응 관계

대부분의 HTML 어트리뷰트는 HTML 어트리뷰트 이름과 동일한 DOM 프로퍼티와 1:1로 대응한다. 언제나 1:1로 대응하는 것은 아니고 어트리뷰트 이름과 프로퍼티 키가 반드시 일치하는 것도 아니다.

- id 어트리뷰트와 id 프로퍼티는 1:1 대응하며, 등일한 값으로 연동한다.
- input 요소의 value 어트리뷰트는 value 프로퍼티와 1:1 대응한다. 하지만 value 어트리뷰트는 초기 상태, value 프로퍼티는 최신 상태를 갖는다.
...
data 어트리뷰트와 dataset 프로퍼티를 사용하면 사용자 정의 어트리뷰트와 자바스크립트 간에 데이터를 교환할 수 있다. data 어트리뷰트는 data-user-id, data-role과 같이 data- 다음에 임의의 이름을 붙여 사용한다.
...
data 어트리뷰트의 값은 HTMLElement.dataset 프로퍼티로 얻을 수 있다. dataset 프로퍼티는 HTML 요소의 모든 data 어트리뷰트의 정보를 제공하는 DOMStringMap 객체를 반환한다. 이 객체는 data- 의 이름을 카멜 케이스로 변환한 프로퍼티를 가지고 있다. 이 프로퍼티로 data 어트리뷰트의 값을 얻거나 변경할 수 있다.
...
data 어트리뷰트에 data- 다음 존재하지 않는 이름을 키로 사용하면 dataset 프로퍼티에 할당하면 HTML 요소에 data 어트리뷰트가 추가된다. 이때 카멜 케이스의 프로퍼티 키는 케밥 케이스로 자동 변경되어 추가된다.
...
![image-20220322205625658](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322205625658.png)



style 프로퍼티를 참조하면 CSSStyleDeclaration 타입의 객체를 반환한다. 이 객체는 CSS 프로퍼티에 대응하는 프로퍼티를 가지고 있으며, 이 프로퍼티에 값을 할당하면 해당 CSS 프로퍼티가 인라인 스타일로 HTML 요소에 추가되거나 변경된다.
...
단위 지정이 필요한 CSS 프로퍼티의 값은 반드시 단위를 지정해야 한다. 단위를 지정하지 않으면 CSS 프로퍼티는 적용되지 않는다.



#### 39.8.2 클래스 조작

.으로 시작하는 클래스 선택자를 사용하여 CSS class를 미리 정의한 다음, HTML 요소의 class 어트리뷰트값을 변경하여 HTML 요소의 스타일을 변경할 수도 있다.
...
Element.prototype.className 프로핕는 setter와 getter 모두 가진 접근자 프로퍼티다.
...
![image-20220322210526330](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322210526330.png)
...
DOMTokenList 객체는 class 어트리뷰트의 정보를 나타내는 컬렉션 객체로서 유사 배열 객체이면서 이터러블이다. 이 객체는 다음과 같은 유용한 메서드들을 제공한다.
...
- remove(...className)
  ...
  remove 메서드는 인수로 전달한 1개 이상의 문자열과 일치하는 클래스를 class 어트리뷰트에서 삭제한다. 일치하는 클래스가 없으면 무시된다.
...
- item(index)
  ...
  item 메서드는 인수로 전달한 index에 해당하는 클래스를 반환한다.
...
- replace(oldClassName, newClassName);
  ...
  replace 메서드는 oldClassName을 newClassName으로 변경한다.

  ```js
  $box.classList.replace('red', 'blue');	// red를 blue로 변경
  ```

- toggle(className[, force])

  toggle 메서드는 인수로 전달한 문자열과 일치하는 클래스가 존재하면 제거하고, 존재하지 않으면 추가한다.

  ```js
  $box.classList.toggle('foo');	// 추가됨
  $box.classList.toggle('foo');	// 제거됨
  ```

  두 번째 인수로 불리언 값으로 평가되는 조건식을 전달할 수 있다. 조건식의 결과가 true면 어트리뷰트에 첫 번째 인수로 전달받은 문자열을 추가하고, false 이면 강제로 전달받은 문자열을 제거한다.

  ```js
  $box.classList.toggle('foo', true);		// 강제로 추가
  $box.classList.toggle('foo', false);	// 강제로 제거
  ```
...
style 프로퍼티는 인라인 스타일만 반환한다. 따라서 클래스를 적용한 스타일이나 상속을 통해 암묵적으로 적용된 스타일은 참조할 수 없다. HTML 요소에 적용되어 있는 모든 CSS 스타일을 참조해야 할 경우 getComputedStyle 메서드를 사용한다.

window.getCoumputedStyle(element[, pseudo]) 메서드는 첫 번째 인수로 전달한 요소 노드에 적용되어 있는 평가된 스타일을 CSSStyleDeclaration 객체에 담아 반환한다. 평가된 스타일이란 지금 현재 요소에 적용돼 있는 모든 스타일을 말한다.
...
![image-20220322212311658](C:\Users\Sanghyun\AppData\Roaming\Typora\typora-user-images\image-20220322212311658.png)

getComputedStyle 메서드의 두 번째 인수(pseudo)로 :after, :before와 같은 의사 요소를 지정하는 문자열을 전달할 수 있다. 의사 요소가 아닌 일반 요소의 경우 두 번째 인수를 생략한다.
...
만약 특정 타입의 이벤트에 반응하여 어떤 일을 하고 싶다면 호출될 함수를 브라우저에게 알려 호출을 위임한다.

- 이벤트 핸들러: 이벤트가 발생했을 때 호출될 함수
- 이벤트 핸들러 등록: 이벤트가 발생했을 때 브라우저에게 이벤트 핸들러의 호출을 위임하는 것

브라우저는 사용자의 이벤트(버튼 클릭 등)를 감지할 수 있지만 우리는 안되기 때문에 이벤트 호출을 브라우저에게 위임하는 것이다.
...
위와 같은 프로그램의 흐름을 이벤트 중심으로 제어하는 프로그래밍 방식을 **이벤트 드리븐 프로그래밍**이라 한다.



### 40.2 이벤트 타입

이벤트 타입은 약 200여 가지가 있고, 사용 빈도가 높은 이벤트들은 다음과 같다.
...
| 이벤트 타입 | 이벤트 발생 시점                                             |
| ----------- | ------------------------------------------------------------ |
| keydown     | 모든 키를 눌렀을 때 발생한다.<br />※ 문자, 숫자, 특수 문자, enter 키를 눌렀을 때는 연속적으로 발생하지만 그 외의 키를 눌렀을 때는 한 번만 발생한다. |
| keypress    | 문자 키를 눌렀을 때 연속적으로 발생한다.<br />※ 문자, 숫자, 특수 문자, enter 키를 눌렀을 때만 발생한다. 폐지되었으니 사용하지 않는 것을 권장한다. |
| keyup       | 누르고 있던 키를 놓았을 때 한 번만 발생한다.<br />※ keydown 이벤트와 같은 키들이 해당된다. |
...
focusin, focusout 이벤트는 addEventListener 메서드 방식을 사용해야 정삭 동작 한다.
...
| 이벤트 타입 | 이벤트 발생 시점                                             |
| ----------- | ------------------------------------------------------------ |
| submit      | 1. form 요소 내의 input, select 입력 필드(textarea 제외)에서 엔터 키를 눌렀을 때<br />2. form 요소 내의 submit 버튼을 클릭했을 때 |
...
HTML 요소의 어트리뷰트 중에는 이벤트 핸들러 어트리뷰트가 있다. 어트리뷰트 값으로 함수 호출문 등의 문을 할당하면 이벤트 핸들러가 등록된다.
...
주의할 것은 함수 참조가 아닌 문을 할당한다는 것이다. 함수 호출문을 등록하면 함수 호출문의 평가 결과가 이벤트 핸들러로 등록된다.

하지만 위 예제는 어트리뷰트 값으로 함수 호출문을 할당했다. 이때 **이벤트 핸들러 어트리뷰트 값은 암묵적으로 생성될 이벤트 핸들러의 함수 몸체를 의미한다.** 다시 말해, onclick="sayHi('Lee')" 어트리뷰트는 파싱되어 다음과 같은 함수를 암묵적으로 생성하고, 이벤트 핸들러 어트리뷰트 이름과 동일한 키 onclick 이벤트 핸들러 프로퍼티에 할당한다.
...
이처럼 동작하는 이유는 이벤트 핸들러에 인수를 전달하기 위해서다. 만약 어트리뷰트 값으로 함수 참조를 할당해야 한다면 인수를 전달하기가 곤란하다.
...
어트리뷰트 값으로 할당한 문자열은 암묵적으로 생성되는 이벤트 핸들러의 함수 몸체다. 따라서 여러 개의 문을 할당할 수 있다.
...
오래된 코드에 간혹 이 방식을 사용한 것이 있기 때문에 알아둘 필요는 있으나 사용하지 않는 것이 좋다.



#### 40.3.2 이벤트 핸들러 프로퍼티 방식

DOM 노드 객체는 이벤트에 대응하는 이벤트 핸들러 프로퍼티를 가지고 있다. 프로퍼티의 키는 onclick과 같이 on + 이벤트 타입으로 이루어져 있다.
...
이벤트 핸들러를 등록하기 위해서는 이벤트를 발생시킬 객체인 이벤트 타깃($button), 이벤트 종류를 나타내는 이벤트 타입(onclick), 그리고 이벤트 핸들로(함수)를 지정할 필요가 있다.



이벤트 핸들러 프로퍼티 방식은 어트리뷰트와 다르게 HTML과 자바스크립트가 뒤섞이는 문제가 없지만, 이벤트 핸들러 프로퍼티에 하나의 이벤트 핸들러만 바인딩할 수 있다는 단점이 있다.
...
<html>
    ...
    <script>
        ...
        // 이벤트 핸들러 프로퍼티에 이벤트 핸들러를 바인딩
        // 첫 번째로 바인딩된 핸들러는 두 번째로 바인딩된 핸들러에 의해 재할당되어 실행되지 않는다.
        $button.onclick = function(){
...
// useCapture: capture 사용 여부(true/false)
```

addEventListener 메서드의 eventType 문자열에는 on 접두사를 붙이지 않는다.
...
addEventListener 방식은 이벤트 핸들러 프로퍼티에 바인딩된 이벤트 핸들러에 아무런 영향을 주지 않는다. 따라서 하나 이상의 이벤트 핸들러를 등록할 수 있다. 이때 이벤트 핸들러는 등록된 순서대로 호출된다.
...
EventTarget.prototype.removeEventListener 메서드를 사용해 이벤트 핸들러를 제거할 수 있다. 전달할 인수는 addEventListener와 동일하다. 만약 전달한 인수가 다르다면 제거되지 않는다.
...
removeEventListener 메서드의 인수로 전달한 이벤트 핸들러는 addEventListener와 같아야 하기 때문에 무명 함수를 이벤트 핸들러로 등록한 경우 제거할 수 없다.
...
기명 이벤트 핸들러 내부에서 이벤트 핸들러를 제거하는 것은 가능하다. 이때 이벤트는 한 번만 호출된다.

```js
$button.addEventListener('click', function foo(){
    console.log('button click');
    
    // 이벤트 핸들러를 제거한다. 따라서 이벤트 핸들러가 단 한번만 호출된다.
...
이벤트 핸들러 프로퍼티 방식으로 등록한 이벤트 핸들러는 removeEventListener 메서드로 제거할 수 없다. 이 경우에는 프로퍼티에 null을 할당한다.
...
이벤트가 발생하면 이벤트에 관련한 정보를 담고 있는 이벤트 객체가 동적으로 생성된다. **생성된 이벤트 객체는 이벤트 핸들러의 첫 번째 인수로 전달된다.**
...
클릭 이벤트에 의해 생성된 이벤트 객체는 이벤트 핸들러의 첫 번째 인수로 전달되어 e에 암묵적으로 할당된다. 따라서 이벤트 객체를 전달받으려면 이벤트 객체를 전달받은 매개변수를 명시적으로 선언해야 한다.

어트리뷰트 방식으로 이벤트 핸들러를 등록했다면 다음과 같이 event를 통해 이벤트 객체를 전달받을 수 있다.
...
이벤트 핸들러 어트리뷰트 방식의 경우 이벤트 객체를 전달받으려면 첫 번쨰 매개변수 이름이 반드시 event이어야 한다. 만약 event가 아닌 다른 이름으로 선언하면 이벤트 객체를 전달받지 못한다.



#### 40.5.1 이벤트 객체의 상속 구조

이벤트가 발생하면 이벤트 타입에 따라 다양한 타입의 이벤트 객체가 생성된다. 이 객체들은 상속 구조를 가지고 있다.

Event 인터페이스는 DOM 내에서 발생한 이벤트에 의해 생성되는 이벤트 객체를 나타낸다. 인터페이스에는 모든 이벤트 객체의 공통 프로퍼티가 정의되어 있고 하위 인터페이스에는 이벤트 타입에 따라 고유한 프로퍼티가 정의되어 있다.

위와 같은 이유로 이벤트 객체의 프로퍼티는 발생한 이벤트 타입에 따라 달라진다.
...
| cancelable       | preventDefault 메서드를 호출하여 이벤트의 기본 동작을 취소할 수 있는지 여부. 아래 이벤트는 cancelable: false로 취소할 수 없다.<br />포커스 이벤트: focus/blur<br />리소스 이벤트: load/unload/abort/error<br />마우스 이벤트: dblclick/mouseenter/mouseleave | boolean       |
| defaultPrevented | preventDefault 메서드를 호출하여 이벤트를 취소했는지 여부    | boolean       |
| isTrusted        | 사용자의 행위에 의해 발생한 이벤트인지 여부. 메서드를 통해 인위적으로 발생시킨 이벤트인 경우 isTrusted는 false다. | boolean       |
...
체크박스 요소의 체크 상태가 변경되면 checked 프로퍼티의 값이 변경되고 change 이벤트가 발생한다. 이때 Event 타입의 객체가 생성되고, target 프로퍼티는 이벤트를 발생시킨 객체($checkbox)를 나타낸다.

이벤트 객체의 currentTarget 프로퍼티는 이벤트 핸들러가 바인딩된 DOM 요소($checkbox)를 가리킨다.

위 예제처럼 일반적으로 target 프로퍼티와 currentTarget 프로퍼티는 동일한 DOM 요소를 가리키지만, 다른 요소를 가리킬 수도 있다.
...
<html>
    ...
    <body>
        ...
        <script>
            ...
            // mousemove 이벤트 핸들러
            const move = e =>{
                // 오프셋 = 현재 마우스 포인터 위치 - 드래그 시작 지점의 마우스 포인터 위치
...
키보드 이벤트가 발생하면 생성되는 KeyboardEvent 타입의 이벤트 객체는 altKey, ctrlKey, shiftKey, metaKey, key, keyCode 같은 고유의 프로퍼티를 갖는다.
...
DOM 트리 상에 존재하는 요소 노드에서 발생한 이벤트는 DOM 트리를 통해 전파된다. 이를 이벤트 전파라 한다.
...
ul 요소의 li 요소를 클릭하면 클릭 이벤트가 발생한다. 이때 생성된 이벤트 객체는 이벤트를 발생시킨 요소인 이벤트 타깃을 중심으로 DOM 트리를 통해 전파된다. 이벤트 전파는 다음과 같이 3단계로 구분할 수 있다.
...
<html>
    <body>
        ...
        <script>
            ...
            $frtuis.addEventListener('click', e =>{
                console.log(`이벤트 단계: ${e.eventPhase}`);	// 3: 버블링 단계
                console.log(`이벤트 타깃: ${e.target}`);		// [object HTMLIElement]
                console.log(`커런트 타깃: ${e.currentTarget}`);	// [object HTML]HTMLUListElement
            })
        </script>
    </body>
</html>
...
li 요소를 클릭하면 이벤트가 발생하여 클릭 이벤트 객체가 생성되고 li 요소가 이벤트 타깃이 된다. 이때 클릭 이벤트 객체는 window에서 시작해서 이벤트 타깃 방향으로 전파된다(캡처링 단계). 이후 이벤트 객체가 타깃에 도달한다(타깃 단계). 이후 이벤트 객체는 다시  window 방향으로 전파된다(버블링 단계).

이벤트 핸들러 어트리뷰트/프로퍼티 방식으로 등록한 이벤트 핸들러는 타깃 단계와 버블링 단계의 이벤트만 캐치할 수 있다. addEventListener 메서드는 3번째 인수로 true를 전달하면 캡처링 단계도  캐치할 수 있다.
...
위 예제처럼 이벤트는 이벤트를 발생시킨 이벤트 타깃은 물론 상위 DOM 에서도 캐치할 수 있다.
...
body 요소는 버블링 단계의 이벤트만을 캐치하고 p 요소는 캡처링 단계의 이벤트만 캐치한다. 만약 button 요소에서 클릭 이벤트가 발생하면 먼저 캡처링 단계를 캐치하는 p 요소의 이벤트 핸들러가 호출되고, 그후 버블링 단계의 이벤트를 캐치하는 body 요소의 이벤트 핸들러가 순차적으로 호출된다.
...
사용자가 내비게이션 아이템을 클릭하면 선택된 아이템에 active 클래스를 추가하고 그 외에 다른 아이템의 active 클래스를 제거하는 예제이다.
...
<html>
    ...
    <body>
        ...
        <div>
            선택된 내비게이션 아이템: <em class="msg">apple</em>
        </div>
        <script>
        	const $fruits = document.getElementById('fruits');
...
이 예제의 경우 li 요소가 100개라면 100개의 이벤트 핸들러를 등록해야 한다.

이벤트 위임이란 여러 개의 하위 DOM 요소에 각각 이벤트 핸들러를 등록하는 대신 하나의 상위 DOM 요소에 이벤트 핸들러를 등록하는 방법을 말한다. 이벤트 전파에서도 봤듯이 상위 요소에서도 하위 이벤트를 캐치 할 수 있기 때문에 이것이 가능한 것이다.
...
이때 주의할 점은 상위 요소에 이벤트 핸들러를 등록하기 때문에 실제 이벤트를 발생시킨 요소가 개발자가 기대한 요소와 다를 수 있다는 것이다. 위 예제의 경우 li요소가 아닌 ul#fruits 요소의 하위 요소들중 하나(이벤트 대상이 아닌)를 클릭해도 이벤트가 발생하기 때문에 이를 방지해줄 필요가 있다.
...
DOM 요소는 저마다 기본 동작이 있다. a 요소를 클릭하면 href 어트리뷰트에 지정된 링크로 이동한다 등이다.
...
이벤트 핸들러 어트리뷰트 값으로 지정한 문자열은 암묵적으로 생성되는 이벤트 핸들러의 문(생성될 이벤트 함수의 몸체부분)이다. 따라서 handleClick 함수는 이벤트 핸들러에 의해 일반 함수로 호출된다. 일반 함수로 호출되는 함수 내부의 this는 전역 객체이기 때문에 window를 가리키게 된다.
...
이벤트 핸들러를 화살표 함수로 정의할 경우 화살표 함수는 상위 스코프의 this를 가리키기 때문에 window를 가리키게 된다.
...
위 예제에서 에러가 난 부분인 this.$button.textContent를 보면 increase()는 이벤트 핸들러기 때문에 내부의 this는 DOM 요소(this.$button)를 가리키게 된다. 그럼 this.$button.textContent는 this.$button.$button.textContent가 되기 때문에 에러가 발생한다. 따라서 이벤트 핸들러를 바인딩할 때 bind 메서드를 사용해 this를 전달하여 increase 메서드 내부의 this가 인스턴스를 가리키도록 해야한다.
...
또는 클래스 필드에 할당한 화살표 함수를 이벤트 핸들러로 등록하여 this가 인스턴스를 가리키도록 할 수도 있다. 다만 이 경우 increase는 프로토타입 메서드가 아닌 인스턴스 메서드가 된다.



### 40.10 이벤트 핸들러에 인수 전달

이벤트 핸들러 어트리뷰트의 경우는 인수를 전달할 수 있지만 프로퍼티 방식과 addEventListener의 경우 인수를 전달할 수 없다. 그러나 다음과 같은 방법으로 인수를 전달할 수 있다.
...
이벤트가 발생하면 암묵적으로 Event, UIEvent, MouseEvent같은 객체가 생성된다. 하지만 이벤트 생성자 함수를 호출하여 명시적으로 생성한 이벤트 객체는 임의의 이벤트 타입을 지정할 수 있다. 이를 커스텀 이벤트라 한다.

이벤트 생성자 함수는 첫 번째 인수로 이벤트 타입을 나타내는 문자열을 전달받는다. 이때 이벤트 타입은 기존의 것을 사용할 수도 있고 임의의 문자열을 사용할 수도 있다. 이때는 일반적으로 CustomEvent 생성자 함수를 사용한다.
...
커스텀 이벤트 객체는 bubbles, cancelable 이외에도 이벤트 객체 고유의 프로퍼티 값을 지정할 수 있다. 똑같이 두 번째 인수로 프로퍼티를 전달한다.
...
생성된 커스텀 이벤트는 dispatchEvent 메서드로 디스패치(이벤트를 발생시키는 행위)를 할 수 있다. dispatchEvent 메서드에 이벤트 객체를 인수로 전달하면서 호출하면 전달한 이벤트 타입의 이벤트가 발생한다.
...
일반적으로 이벤트 핸들러는 비동기 방식으로 동작하지만 dispatchEvent 메서드는 동기 방식으로 작동한다.



커스텀 이벤트의 경우 일반적으로 CustomEvent 생성자 함수를 사용한다. 만약 이벤트와 함께 전달하고 싶은 정보가 있을 경우 detail 프로퍼티를 포함하는 객체를 두 번째 인수로 전달하면 된다.
...
기존 타입이 아닌 커스텀 이벤트 객체를 생선한 경우 반드시 addEventListener 메서드 방식으로 이벤트 핸들러를 등록해야 한다. 그 이유는 어트리뷰트나 프로퍼티 방식의 경우 요소 노드가 존재하지 않기 때문이다.
